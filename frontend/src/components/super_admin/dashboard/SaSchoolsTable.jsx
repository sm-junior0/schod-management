      ...
      <table className="w-full">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">School Admin</th>
            <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">School Name</th>
            <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">School Email</th>
          ...
          {schools.map((school) => (
            <tr key={school.id} className="border-t border-gray-100">
              <td className="px-4 py-3">
                <img src={school.logo} alt={`${school.schoolName} logo`} className="w-8 h-8 rounded-full" />
              </td>
              ...
              <td className="px-4 py-3">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                  ${school.status === 'Approved' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  {school.status}
                </span>
              </td>
              <td className="px-4 py-3">
